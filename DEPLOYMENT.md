...
### Вариант 1: Запуск локально (РЕКОМЕНДУЕТСЯ для начала)

**Цель:** Запустить проект на вашем компьютере, протестировать, убедиться что все работает.

```bash
# Шаг 1: Установить Docker Desktop (если еще не установлен)
...
cd apps/backend
cp .env.example .env
# Отредактировать .env (см. раздел "Настройка окружения" ниже)
pnpm prisma generate
pnpm prisma migrate dev
pnpm prisma db seed
pnpm dev

# Шаг 4: В новом терминале - запустить frontend
cd apps/frontend
pnpm dev
...
**После этого переходите к Варианту 2 (Production deployment).**
...
1. **Подготовка сервера** → см. раздел "Option 1: VPS Deployment" ниже
2. **Настройка домена** → настроить DNS на IP сервера
3. **Установка проекта** → клонировать репозиторий на сервер
4. **Настройка окружения** → создать production .env файлы
5. **Запуск баз данных** → docker-compose up -d
6. **Запуск приложения** → PM2 или Docker
7. **Настройка Nginx** → reverse proxy + SSL
